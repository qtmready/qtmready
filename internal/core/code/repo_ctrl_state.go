...
func (state *RepoCtrlState) on_push(ctx workflow.Context) defs.ChannelHandler {
	return func(rx workflow.ReceiveChannel, more bool) {
		...
		state.StashedEvents.push(branch, event)
	}
}
...
func (state *RepoCtrlState) on_branch_create(ctx workflow.Context, event *defs.Event[defs.BranchOrTag, defs.RepoProvider]) {
	...
	events, ok := state.StashedEvents.all(event.Payload.Ref)
	...
	if ok {
		...
		state.StashedEvents.clear(event.Payload.Ref)
	}
}
...
func NewRepoCtrlState(ctx context.Context, repo *defs.Repo, info *defs.RepoIOProviderInfo) *RepoCtrlState {
	return &RepoCtrlState{
		BaseState:     NewBaseState(ctx, "repo_ctrl", repo, info, ""),
		StashedEvents: make(StashedPushEvents[defs.RepoProvider]),
